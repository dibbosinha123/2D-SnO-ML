...
# Modified filtering to include normal SnO bandgap range
df["nelements"] = df["elements"].apply(len)
df = df[df["nelements"].isin([2, 3])]
df = df.dropna(subset=["band_gap"])

# Improved 2D vs normal SnO classification
...
df["structure_type"] = df.apply(lambda x: "2D SnO" if (
...
# ============================================================
print("\n" + "="*50)
print(" BANDGAP ANALYSIS: 2D SnO vs Normal SnO")
print("="*50)
...
joblib.dump(best_model, f"best_model_SnO.pkl")
...
print("-"*50)
for struct_type in ["2D SnO", "Normal SnO"]:
